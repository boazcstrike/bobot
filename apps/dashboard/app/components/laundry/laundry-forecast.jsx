...
import ForecastLoadChart from "./forecast-load-chart";
import {
  Sheet,
  SheetClose,
  SheetContent,
...
function formatMonth(yyyymm) {
  return toDate(`${yyyymm}-01`).toLocaleDateString(undefined, { month: "long", year: "numeric" });
}

function countdownLabel(daysUntilNext) {
...
function ForecastFormula({ forecast }) {
 ...
 */
export default function LaundryForecast({ forecast, intervalHistory = [] }) {
  ...
  const monthlyOutlook = useMemo(() => {
    ...
    for (const point of projection) {
      if (!byMonth.has(point.month)) {
        byMonth.set(point.month, { month: point.month, count: 0, firstDate: point.date });
      }
      byMonth.get(point.month).count += 1;
    }
  ...
  const cadenceLow = Math.max(1, averageGapDays - Math.round(forecast.stdDevDays));
  const cadenceHigh = Math.max(averageGapDays, averageGapDays + Math.round(forecast.stdDevDays));
        ...
        <div className="forecast-chart-block">
          ...
          <ChartContainer config={cadenceConfig} className="aspect-auto h-[180px] w-full">
            ...
            >
              ...
              <ReferenceArea y1={cadenceLow} y2={cadenceHigh} fill="var(--chart-5)" fillOpacity={0.15} stroke="none" />
              <ReferenceLine y={averageGapDays} stroke="var(--chart-5)" strokeDasharray="5 4" strokeWidth={2} />
              <Area
                dataKey="gapDays"
                type="monotone"
